...
class TrainTask(BaseTask, VisualizeTaskMixin, EvalTaskMixin):
    ...
    def __init__(self, model, datasets, cfg):
        super(TrainTask, self).__init__(model, datasets, cfg)
        ...
        if self.cfg.task.lr_scheduler is not None:
            schedulers = {
                "step": torch.optim.lr_scheduler.StepLR(
                    self.opt, step_size=cfg.task.lr_scheduler_step
                ),
                ...
                "cosann": torch.optim.lr_scheduler.SequentialLR(
                    self.opt,
                    schedulers=[
                        ...
                        torch.optim.lr_scheduler.CosineAnnealingLR(
                            ...
                            eta_min=cfg.task.linear_scheduler_start_factor
                            * cfg.task.lr,
    ...
    def training_throughput(self):
        last_train_step_duration = (
            self.end_train_step_timestamp - self.start_train_step_timestamp
        ).total_seconds()
    ...
    def _load_state_from_ckpt(self):
        ckpt = torch.load(
            os.path.join(
                hydra.utils.get_original_cwd(), f"../checkpoints/{self.cfg.model.ckpt}"
            ),
    ...
    def get_cyclic_scheduler_last_epoch(self):
        ...
        if self.restored_run_final_it == 0:
            ...
            return self.restored_run_final_it % (
                self.cfg.task.cyclic_scheduler_step_up
                + self.cfg.task.cyclic_scheduler_step_up
            )
    ...
    def _simple_serialize_state(self, loss_step, it):
        goal_maximize = True if "acc" in self.cfg.task.early_stop_metric else False
        if goal_maximize:
            has_improved = (self.best_valid_score is None) or (
                loss_step > self.best_valid_score
            )
        else:
            has_improved = (self.best_valid_score is None) or (
                loss_step < self.best_valid_score
...
class EarlyStopping:
    ...
    def __call__(self, val_metric, model, opt, it):
        if self.val_metric_best == np.Inf:
            ...
            self.counter += 1
            self.trace_func(
                f"EarlyStopping counter: {self.counter} out of {self.patience}. {val_metric} did not push avg {np.mean(self.past_metrics)}."
            )
    ...
    def save_checkpoint(self, val_metric, model, opt, it):
        ...
        if self.verbose:
            self.trace_func(
                f"Validation metric improved ({self.val_metric_best:.6f} --> {val_metric:.6f}).  Saving model ..."
            )
        torch.save(
            {
                "update": self.restored_run_final_it + it,
