...
class VisualizeTaskMixin:
	...
	def store_errors_for_log(self, input, output, target, pred_is_exact, dataset_name, max_errors=2):
		...
		count_added = 0
		for input, output, target, is_exact in zip(input_str, output_str, target_str, pred_is_exact):
			output = self.vocabulary.cut_at_first_eos(output)
			if not is_exact and (count_added < max_errors):
	...
	def add_weights_norm_to_metrics_dict(self):
		...
		self.valid_step_metrics['stats/model/weights_norm'] = weights_norm
	...
	def log_n_model_params(self):
		...
		wandb.log({'stats/model/num_parameters': n_model_params})
		logging.info(f'Num. model parameters: {n_model_params}')


	def plot_cross_attention_values(self, cross_attention_values, samples_ids, samples_strings):
		logging.info('Plotting cross attention values...')
		model_id = f"{len(self.model.encoder_layers)}l_{self.model.encoder_layers[-1].diag_mask_width_above}w"
		for sample_id, cross_attention, sample_string in zip(samples_ids, cross_attention_values, samples_strings):
			cross_attention = cross_attention[:len(sample_string)].detach().cpu()
			fig, ax = plt.subplots()
			ax.bar(range(len(sample_string)), cross_attention)
			ax.set_ylim(0, .4)
			ax.set_xticks(range(len(sample_string)), sample_string, rotation=0)
			ax.set_title(model_id)
			plt.savefig(f'../out/plots/{model_id}/cross_attention_{sample_id}.png')
			plt.close()