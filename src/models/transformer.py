...
class Transformer(torch.nn.Module):
    ...
    def __init__(
            self, d_model, ff_mul, num_heads, num_layers_enc, num_layers_dec, vocabulary, label_pe_enc=False,
            label_pe_dec=False,
            ...
            mha_init_gain=1, num_recurrent_steps=1, multi_fwd_threshold=-1, dropout=0.1, use_pe_enc=True, device='cuda'):
        ...
        for _ in range(num_layers_enc):
            self.encoder_layers.append(
                Encoder(d_model, ff_mul, num_heads, dropout=dropout, label_pe=label_pe_enc, max_range_pe=max_range_pe,
                        ...
                        average_attn_weights=average_attn_weights, store_attn_weights=store_attn_weights, use_pe=use_pe_enc,
                        device=device))
        ...
        self.x_emb = Embedding(num_embeddings=self.len_x_vocab, embedding_dim=self.d_model, padding_idx=self.idx_PAD_x,
                               device=self.device)
        self.y_emb = Embedding(num_embeddings=self.len_y_vocab, embedding_dim=self.d_model, padding_idx=self.idx_PAD_y,
                               device=self.device)
    ...
    def load_model_weights(self, ckpt):
        ...
        torch_ckpt = torch.load(os.path.join(hydra.utils.get_original_cwd(), f'../checkpoints/{ckpt}'),
    ...
    def forward(self, X, Y=None, tf=False, enc_widths=None):
        ...
        if self.n_multi is not None:
            ...
            return self._fwd(X, Y, tf=tf, enc_widths=enc_widths)
        else:
            return self._test_fwd(X)

    def _fwd(self, X, Y, tf=False, enc_widths=None):
        ...
        if not tf:
            ...
            output = F.one_hot(Y_greedy_pred_tokens, num_classes=self.len_y_vocab).type(torch.FloatTensor).to(
                X.device)  # placeholder
            ...
            Y = self.y_emb(Y)
            X, src_mask = self._encoder(X, src_mask, widths=None)
    ...
    def _test_fwd(self, X, enc_widths=None):
        it, max_it = 0, 100
        src_mask = (X == self.idx_PAD_x)
        X = self.x_emb(X)
        encoding, src_mask = self._encoder(X, src_mask, widths=enc_widths)
        stopped = torch.zeros(X.size(0)).type(torch.BoolTensor).to(X.device)
        output = torch.tile(F.one_hot(torch.tensor([self.idx_SOS_y]), num_classes=self.len_y_vocab),
                            dims=(X.size(0), 1, 1)).type(torch.FloatTensor).to(X.device)
        ...
        while not stopped.all() and (it < max_it):
            ...
            if self.deterministic:
                ...
                tokens_distrib = F.softmax(Y_pred.squeeze(1) / self.temperature,
                                           dim=-1)  # we can squeeze bc we take 1 sample
        ...
        return TransformerTestOutput(logits=output[:, 1:, :], proba=output_proba, pad_idx=self.idx_PAD_y,
                                     eos_idx=self.idx_EOS_y)
    ...
    def _update_outputs_log_proba(self, outputs_log_proba, X, argmax_solver_out, multi_idx, halted):
        ...
        if halted.all():
            ...
            outputs_log_proba[~halted, multi_idx] = output_log_proba_last_batch[~halted].sum(
                -1)  # / outputs_lengths[~halted]
            outputs_log_proba[~halted, multi_idx] = torch.nan_to_num(outputs_log_proba[~halted, multi_idx], nan=0,
                                                                     posinf=0, neginf=0)
    ...
    def _update_finder_scores(self, finder_scores, X, solver_out, multi_idx, halted):
        ...
        if halted.all():
            ...
            finder_scores[~halted, multi_idx] = self.combiner.finder.get_expressions_match(X[~halted],
                                                                                           splitted_expressions[~halted,
                                                                                           :expressions_lengths[
                                                                                               splittable].max()])
        ...
        if (not halted.all()) and (not splittable_or_halted.all()):
            ...
            finder_scores[~halted, multi_idx] = torch.nan_to_num(finder_scores[~halted, multi_idx], nan=0, posinf=0,
                                                                 neginf=0)
    ...
    def _multi_fwd(self, X):
        ...
        for multi_idx in range(self.n_multi):
            ...
            outputs_log_proba = self._update_outputs_log_proba(outputs_log_proba, X, argmax_solver_out, multi_idx,
                                                               halted)
        ...
        for input_idx, outputs_per_input in enumerate(multi_outputs_T):
            ...
            avg_outputs_log_probas = {k: v for k, v in
                                      sorted(avg_outputs_log_probas.items(), key=lambda item: item[1], reverse=True)}
            counter_valid_outputs = Counter(
                [output for output in outputs_per_input if output in outputs_with_finder_score_1])
            logging.info('')
...
class Encoder(torch.nn.Module):
    ...
    def __init__(self, d_model, ff_mul, num_heads, dropout=0.1, label_pe=False, max_range_pe=1000,
                 diag_mask_width_below=None, diag_mask_width_above=None, average_attn_weights=True,
                 store_attn_weights=False, use_pe=True, device='cpu'):
    ...
    def forward(self, X, src_mask, widths=None):
        if not self.use_pe:
            pass
        else:
            X = self._pe(X)
        X = self._encoder(X, src_mask, widths)
        return X
    ...
    def _encoder(self, X, src_mask, widths=None):
        if self.diag_mask_width_below is not None:
            ...
            attn_mask = _gen_mask_window_around_diag(src_mask=src_mask, num_heads=self.MHSA.num_heads,
                                                     below=self.diag_mask_width_below,
                                                     above=self.diag_mask_width_above).to(X.device)
        elif widths is not None:
            attn_mask = _gen_variable_width_mask_window_around_diag(src_mask=src_mask, num_heads=self.MHSA.num_heads,
                                                                    width_below_above=widths).to(X.device)
        else:
            attn_mask = None
        ...
        Xt, self_attn = self.MHSA(X, X, X, attn_mask=attn_mask, key_padding_mask=src_mask,
...
def _gen_variable_width_mask_window_around_diag(src_mask, num_heads, width_below_above):
    """Generates a mask around the diagonal of the self attention.
    ...
    max_seq_len = src_mask.size(1)
    bs = len(seq_lens)
    assert len(width_below_above) == bs
    mask = torch.ones(bs * num_heads, max_seq_len, max_seq_len)

    width_below_above = width_below_above.unsqueeze(0).tile((num_heads, 1)).flatten()
    unique_widths = width_below_above.unique()
    for width in unique_widths:
        idx_masks_of_width = (width_below_above == width).to(mask.device)
        above = width.type(torch.int).item()
        below = width.type(torch.int).item()
        ...
        for w in range(-below, above + 1, 1):
            mask[idx_masks_of_width] -= torch.diag(torch.ones(max_seq_len - abs(w)), w).unsqueeze(0)
    return mask.bool()
