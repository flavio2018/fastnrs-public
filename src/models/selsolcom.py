...
class SelSolCom(torch.nn.Module):
    ...
    def __init__(
        ...
        #     assert (
        #         self.n_multi % 10 == 0
        #     ), "n_multi must be a multiple of 10 when zoom_selector=True."
    ...
    def forward(self, expression):
        ...
        while torch.bitwise_and(is_valid, ~is_final).any():
            ...
            is_valid = self._update_is_valid_using_finder_score(
                is_valid, sub_expression_obj
            )
            if ~is_valid.any():
                break
            ...
            solution_tmp = self.solver.vocabulary.str_to_batch(
                solution_tmp_str, x=False
            )
            ...
            sub_expression_obj = [
                o for o in sub_expression_obj if o.normalized_finder_score >= 1
            ]
            logging.info(
                "\n".join(
                    ["Valid sub-expressions:"] + [o.text for o in sub_expression_obj]
                )
            )
            ...
            tokenized_expression = [
                self.vocabulary.tokenize_sample(e) for e in expression_str
            ]
            tokenized_solution = [
                self.vocabulary.tokenize_sample(s) for s in solution_str
            ]

            # run combiner to pool solutions into expressions
            new_expression_str = self.combiner(
                tokenized_expression, position, tokenized_solution, sub_expression_len
            )

            # update valid sequences based on the new expression
            is_valid = self._update_is_valid_using_expression(
                is_valid, new_expression_str
            )
            if ~is_valid.any():
                break
            ...
            new_expression_str = [
                e for e in new_expression_str if self._has_well_formed_parentheses(e)
            ]
            ...
            expression = self.get_full_batch(
                expression_tmp,
                is_valid,
                self.selector.vocabulary.get_special_idx("pad"),
            )

        if ~is_valid.any():
            expression = self.get_full_batch(
                expression, is_valid, self.selector.vocabulary.get_special_idx("pad")
    ...
    def _zoom_in_selector_input(self, expression, length_threshold, zoom_perc):
        expression_lengths = (
            expression != self.selector.vocabulary.get_special_idx("pad")
        ).sum(1)
        ...
        size_zoom_window[is_zoomed] = (
            expression_lengths[is_zoomed] // (1 / zoom_perc)
        ).to(torch.int64)
        ...
        for e, zoomed, win_size in zip(expression, is_zoomed, size_zoom_window):
            if zoomed:
                ...
                if (e == self.selector.vocabulary.get_special_idx("pad")).any():
                    position_first_pad = (
                        (e == self.selector.vocabulary.get_special_idx("pad"))
                        .argwhere()[0]
                        .item()
                    )
                    real_start = (
                        position_first_pad - win_size
                        if position_first_pad - win_size >= 0
                        else 0
                    )  # handle case of spurious pad in the middle of seq
                    zoomed_expr_str = self.selector.vocabulary.batch_to_str(
                        e[real_start:position_first_pad].unsqueeze(0)
                    )[0]
                else:
                    zoomed_expr_str = self.selector.vocabulary.batch_to_str(
                        e[-win_size:].unsqueeze(0)
                    )[0]
        ...
        zoomed_expressions = self.selector.vocabulary.str_to_batch(
            list_zoomed_expressions
        )
    ...
    def _selector_multi_run(self, expression):
        ...
        for run_idx in range(self.n_multi):
            ...
            selector_output_str = self.selector.vocabulary.batch_to_str(
                transformer_test_output.tokens, x=False
            )
            selector_output_str = [
                o.replace(self.selector.vocabulary.specials["eos"], "")
                for o in selector_output_str
            ]
            selector_output_str = [
                o.replace(self.selector.vocabulary.specials["hal"], "")
                for o in selector_output_str
            ]
            selector_output_str = [
                o.replace(self.selector.vocabulary.specials["sos"], "")
                for o in selector_output_str
            ]
            selector_output_solve_vocab = self.vocabulary.str_to_batch(
                selector_output_str
            )

            position, finder_score = self.finder(
                expression, selector_output_solve_vocab
            )

            for input_idx in range(batch_size):
                runs_outputs[input_idx].append(
                    SelectorOutput(
                        self.selector.vocabulary.tokenize_sample(
                            selector_output_str[input_idx]
                        ),
        ...
        for input_idx in runs_outputs.keys():
            runs_outputs[input_idx] = sorted(
                runs_outputs[input_idx],
                key=lambda o: (o.normalized_finder_score, o.confidence_score),
                reverse=True,
            )
            logging.info(
                "\n".join(
                    ["5 best multi-run outputs:"]
                    + [str(o) for o in runs_outputs[input_idx][:5]]
                )
            )
            logging.info(
                "\n".join(
                    ["Unique multi-run outputs with perfect finder score:"]
                    + list(
                        set(
                            [
                                o.text
                                for o in runs_outputs[input_idx]
                                if o.normalized_finder_score >= 1
                            ]
                        )
                    )
                )
    ...
    def _selector_multi_run_hq(self, expression):
        ...
        if self.zoom_selector:
            window_frac = 20  # smallest window is 1/window_frac-th of the input
            repeats = self.n_multi // window_frac
            zoom_percentages = np.tile(np.linspace(0.1, 1, num=window_frac), repeats)
        ...
        for count_trials in range(max_trials):
            ...
            if self.zoom_selector:
                expression = self._zoom_in_selector_input(
                    input_expression,
                    length_threshold=self.length_threshold,
                    zoom_perc=zoom_percentages[count_trials],
                )

            transformer_test_output = self.selector(expression)
            confidence_score = self._get_confidence_score(transformer_test_output)
            proba_confidence_score = self._get_confidence_score(
            ...
            for idx, o in enumerate(selector_output_str):
                if len(o) == 0:
                    if (
                        self.vocabulary.task_name_from_sample(
                            self.vocabulary.batch_to_str(input_expression)[0]
                        )
                        == "logic"
                        and len(expression[idx]) == 1
                    ):
                        selector_output_str[idx] = (
                            self.selector.vocabulary.batch_to_str(expression)[idx]
                        )
                    else:
                        selector_output_str[idx] = "#"
            ...
            position_finder, finder_score = self.finder(
                input_expression, selector_output_solve_vocab
            )

            for input_idx in range(batch_size):
                runs_outputs[input_idx].append(
                    SelectorOutput(
                        tokenized_text=self.selector.vocabulary.tokenize_sample(
                            selector_output_str[input_idx]
                        ),
                ...
                if (
                    runs_outputs[input_idx][-1].normalized_finder_score == 1
                ):  # and (confidence_score[input_idx].item() >= -0.001):
                    self.count_hq[input_idx] += 1

                self.conf_score_by_input_len[
                    expression_lengths[input_idx].item()
                ].append(proba_confidence_score[input_idx].item())

        logging.info(
            f"Run {count_trials+1} out of {max_trials} max trials. Found {self.count_hq.mean()} HQ outputs per input on average."
        )
    ...
    def _solver_multi_run_hq(self, sub_expression):
        ...
        while (not is_hq.all()) and (count_trials < max_trials):
            ...
            confidence_score = self._get_confidence_score(transformer_test_output)
            solver_output_str = self.solver.vocabulary.batch_to_str(
                transformer_test_output.tokens, x=False
            )
            solver_output_str = [
                self.solver.vocabulary.cut_at_first_eos(o) for o in solver_output_str
            ]
        ...
        logging.info(
            f"Run {count_trials} out of {max_trials} max trials. Found HQ outputs for {is_hq.sum()}/{batch_size} inputs."
        )

        for input_idx in runs_outputs.keys():
            runs_outputs[input_idx] = sorted(
                runs_outputs[input_idx], key=lambda o: o.confidence_score, reverse=True
            )
            logging.info(
                "\n".join(
                    ["5 best multi-run outputs:"]
    ...
    def get_full_batch(self, valid_batch, is_valid, filler):
        ...
        full_batch = torch.full(
            (batch_size, valid_batch.size(1)), filler, device=valid_batch.device
        )
        if is_valid.any():
    ...
    @staticmethod
    def _get_confidence_score(transformer_test_output, use_proba=False):
        ...
        where_first_eos_fixed = torch.zeros(transformer_test_output.tokens.size(0)).to(
            where_first_eos
        )
        ...
        if use_proba:
            return torch.nan_to_num(
                transformer_test_output.proba.prod(-1), nan=0, posinf=0, neginf=0
            )
        else:
    ...
    def check_invalid_final_state(self, expression):
        open_parenthesis = (
            "(" if self.selector.vocabulary.dataset_name != "listops" else "["
        )
        has_invalid_final_state = (
            expression[:, 0] == self.selector.vocabulary.x_vocab[open_parenthesis]
        )
        if has_invalid_final_state.any():
            expression[has_invalid_final_state, :] = (
                self.selector.vocabulary.get_special_idx("pad")
            )
    ...
    def update_solver_errors_counter(self, solution, sub_expression, is_valid):
        ...
        valid_sub_expr_res = [
            self.eval_sub_expression(s_e) for s_e in valid_sub_expr_str
        ]
        solution_str = self.solver.vocabulary.batch_to_str(solution[is_valid], x=False)
        solution_str = [
            s.replace(self.solver.vocabulary.specials["eos"], "") for s in solution_str
        ]
        solution_str = [
            s.replace("MIN", "").replace("MAX", "").replace("SM", "")
            for s in solution_str
        ]
        if (
            self.solver.vocabulary.dataset_name == "algebra"
            or self.solver.vocabulary.task_name_from_sample(valid_sub_expr_str[0])
            == "algebra"
        ):
            parsed_solution_str = []
            for s in solution_str:
                if s == "$":
                    parsed_solution_str.append(s)
                else:
                    try:
                        parsed_solution_str.append(parse_expr(s) % 100)
                    except (ValueError, SyntaxError):
                        parsed_solution_str.append("$")
            solution_str = parsed_solution_str
        pred_is_wrong = [
            true != pred for true, pred in zip(valid_sub_expr_res, solution_str)
        ]
        for idx, is_wrong in enumerate(pred_is_wrong):
            if is_wrong:
                logging.info(
                    f"[ERRLOG][SOLVER] Input: {valid_sub_expr_str[idx]} Output: {solution_str[idx]} Target: {valid_sub_expr_res[idx]}"
                )

        # if torch.tensor(pred_is_wrong).any():
        #     breakpoint()
        self.count_solver_errors_per_seq[is_valid] += (
            torch.tensor(pred_is_wrong).int().to(sub_expression.device)
        )
        self.run_batch_history.update_solver_errors(
            valid_sub_expr_res, solution_str, is_valid
        )
        return pred_is_wrong

    def _update_is_final(self, is_final, solution):
        is_final_now = solution[:, 0] == self.solver.vocabulary.get_special_idx(
            "hal", x=False
        )
    ...
    def _update_is_valid_using_finder_score(self, is_valid, selector_multi_run_output):
        selector_output_it = iter(
            selector_multi_run_output
        )  # using iter since len(is_valid) != len(selector_multi_run_output) in general
        now_invalid_seq_idx = []
        now_invalid_seqs = []
    ...
    def _update_is_valid_using_expression(self, is_valid, expressions_str):
        expressions_it = iter(
            expressions_str
        )  # using iter since len(is_valid) != len(expressions_str) in general
        now_invalid_seq_idx = []
        now_invalid_seqs = []
        ...
        self.log_invalid_seq(
            now_invalid_seq_idx, now_invalid_seqs, sub_expression=False
        )
    ...
    @staticmethod
    def _subexpr_has_well_formed_parentheses(subexpr_str):
        if subexpr_str.count("[") != subexpr_str.count("]") or (
            subexpr_str.count("[") == subexpr_str.count("]")
            and subexpr_str.count("[") > 1
        ):
            return False
        elif subexpr_str.count("(") != subexpr_str.count(")") or (
            subexpr_str.count("(") == subexpr_str.count(")")
            and subexpr_str.count("(") > 1
        ):
            return False
    ...
    def eval_sub_expression(self, sub_expression):
        ...
        def eval_algebra(sub_expression):
            try:
                parsed_expr = parse_expr(sub_expression)
            except:
                logging.info(f"Error while parsing {sub_expression}!")
                return -100
        ...
        def eval_arithmetic(sub_expression):
        ...
        def eval_logic(sub_expression):
            if "&" in sub_expression and "F" in sub_expression:
                return "F"
            elif "T&T" in sub_expression:
                return "T"
            elif "|" in sub_expression and "T" in sub_expression:
                return "T"
            elif "F|F" in sub_expression:
                return "F"
            elif "!T" in sub_expression:
                return "F"
            elif "!F" in sub_expression:
                return "T"
            elif match := re.match(f"\([a-z]&[a-z]\)", sub_expression):
                return match.string[1]
            elif match := re.match(f"\([a-z]|[a-z]\)", sub_expression):
                return match.string[1]
            elif match := re.match(f"\([a-z]&T\)", sub_expression):
                return match.string[1]
            elif match := re.match(f"\(T&[a-z]\)", sub_expression):
                return match.string[3]
            elif match := re.match(f"\([a-z]|F\)", sub_expression):
                return match.string[1]
            elif match := re.match(f"\(F|[a-z]\)", sub_expression):
                return match.string[3]
            elif re.match(f"[a-zTF]", sub_expression):
                return "$"
            else:
                logging.info(f"Unmatched pattern: {sub_expression}")
                return "€"

        if (
            self.solver.vocabulary.dataset_name == "listops"
            or self.solver.vocabulary.task_name_from_sample(sub_expression) == "listops"
        ):
            return eval_listops(sub_expression)
        elif (
            self.solver.vocabulary.dataset_name == "algebra"
            or self.solver.vocabulary.task_name_from_sample(sub_expression) == "algebra"
        ):
            return eval_algebra(sub_expression)
        elif (
            self.solver.vocabulary.dataset_name == "logic"
            or self.solver.vocabulary.task_name_from_sample(sub_expression) == "logic"
        ):
            return eval_logic(sub_expression)
        else:
            return eval_arithmetic(sub_expression)
