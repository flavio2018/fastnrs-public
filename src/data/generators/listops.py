...
class ListOpsTree:
    ...
    def simplify_one_lvl(self, sample):
        ...
        if sub_expressions != []:
            ...
            if self.simplify_last:
                simplified_sample = replacement.join(
                    sample.rsplit(target, 1))  # substitute at most 1 instance, starting from EOS
            else:
    ...
    def get_solution_chain_stats(self, tree=None):
        ...
        return [(self._compute_depth_expression_str(x), d, x, y) for x, y, d in
                zip(tree.steps, tree.sub_expr, num_digits_in_target_sub_expressions)]


# def _get_random_solution_step(self, tree=None):
# 	if tree is None:
# 		assert self.tree is not None, "Instantiate the tree first."
# 		tree = self
# 	return random.choice(self._get_solution_chain_xy(tree))
...
class ListOpsExpressionGenerator:
    ...
    def generate_samples(self, num_samples, nesting, num_operands, task, split='train', exact=False):
        ...
        if (nesting == 3):  # and (num_operands == 3):
    ...
    def _build_select_step_input(self, samples, step):
        ...
        step_idx = int(step[1])
        for sample in samples:
            X_str.append(sample.steps[step_idx])
