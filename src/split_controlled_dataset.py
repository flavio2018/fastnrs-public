...
def main(cfg):
    ...
    dev_splits_dfs['train'] = upsample_set(dev_splits_dfs['train'], 'train')
    dev_splits_dfs['valid_iid'] = upsample_set(dev_splits_dfs['valid_iid'], 'valid_iid')
    dev_splits_dfs['valid_ood'] = upsample_set(dev_splits_dfs['valid_ood'], 'valid_ood')
...
def expand_valid_ood_set(cfg, ood_samples_df, serialized_ood_samples):
    ...
    sample_str2obj = {str(s): s for s in serialized_ood_samples}
...
def upsample_set(df, split_name):
    logging.info(f"Upsampling {split_name} set")
    ...
    max_count = splits_counts[x_name].max()

    for split in splits_counts.index:
        split_count = splits_counts.loc[split, x_name]
        if split_count != max_count:
            upsample_factor = max_count // split_count - 1
            if split_name == 'valid_ood':
                depth, subexpr_ops = split
                split_samples = df[(df['depth'] == depth) & (df['subexpr_ops'] == subexpr_ops)]
            else:
                nesting, num_operands, extra = split
                split_samples = df[(df['nesting'] == nesting) & (df['num_operands'] == num_operands) & (
                        df['extra'] == extra)]
            df = pd.concat([df] + [split_samples] * upsample_factor)

    logging.info(f"{len(df)} samples.")
    logging.info(df.groupby(groupby_fields, dropna=False).count())
...
def dump_ndr_datasets(samples_dfs, cfg):
    for dev_split_name, samples_df in samples_dfs.items():
        if dev_split_name == 'train':
            samples_df = upsample_set(samples_df, dev_split_name)
        samples_df['Y_ndr'] = samples_df['Y_solve']

        samples_df.loc[(samples_df['nesting'] == 1) & (samples_df['num_operands'] == 1), 'Y_ndr'] = \
            samples_df[(samples_df['nesting'] == 1) & (samples_df['num_operands'] == 1)]['X_solve']

        if dev_split_name in ['train', 'valid_iid', 'valid_ood']:
            samples_df = samples_df[samples_df['extra'] == 'no']  # comment for extended version
    ...
    valid_df = valid_df[valid_df['extra'] == 'no']  # comment for extended version
    ...
    for dev_split_name, samples_df in samples_dfs.items():
        if 'Y_ndr' in samples_df.columns:
